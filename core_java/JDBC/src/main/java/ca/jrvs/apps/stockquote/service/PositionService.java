...
public class PositionService {
    ...
    public Position buy(String symbol, int numberOfShares, double price) {
        if (numberOfShares < 0) {
            logger.warn("WARNING: cannot purchase negative number of shares");
            System.out.println("WARNING: cannot purchase negative number of shares");
            return null;
        }
        ...
        Quote quote = quoteService.getQuoteBySymbol(symbol);

        if (quote == null) {
            logger.error("ERROR: Quote not found for symbol: {}", symbol);
            System.out.println("Error: Quote not found for symbol: " + symbol);
            return null;
        }
    ...
    public void sell(String symbol) {
        Optional<Quote> quoteOptional = quoteService.fetchLatestQuote(symbol);
